...
// Create connection pool for SQL queries
...
ORDER BY ID;`);
  ...
  getEmployeeID = async (firstName, lastName) => {
    const [rows, fields] = await db.query(`SELECT id FROM employee WHERE first_name = ? AND last_name = ?;`, [firstName, lastName]);    
    const id = rows[0].id;
  ...
  updateEmpRole = (employee, role) =>
    db.query(`UPDATE employee SET role_id = ? WHERE id = ?;`, [role, employee]);

  updateEmpManager = (employee, manager) =>
    db.query(`UPDATE employee SET manager_id = ? WHERE id = ?;`, [manager, employee]);
